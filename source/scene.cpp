...
void viewUserProfile(int viewingUser) { // 查看某人的主页
    while (backToMainPage) {
        // 输出界面信息以及交互指南
        clearScreen();
        displayUser(USER::user[viewingUser], false);
        cout << "MENU---" << endl;
        if (viewingUser == currentUser) {
            ...
            cout << "Enter 1 to view all posts by " << USER::user[viewingUser].nickName << "." << endl;
            cout << "Enter 2 to view users " << USER::user[viewingUser].nickName << " is following." << endl;
            cout << "Enter 3 to view " << USER::user[viewingUser].nickName << "'s followers." << endl;
            bool isFollowingViewingUser = USER::user[currentUser].isFollowing(viewingUser);
            if (isFollowingViewingUser == false) {
                cout << "Enter 4 to follow " << USER::user[viewingUser].nickName << "." << endl;
            } else {
                cout << "Enter 4 to unfollow " << USER::user[viewingUser].nickName << "." << endl;
            }
            cout << "Enter 0 to return." << endl;
            cout << "Enter -1 to return to the main page." << endl;
            cout << "---" << endl;
            ...
            if (cmd == 1) {
                ...
                if (isFollowingViewingUser == false)
                    followUser(currentUser, viewingUser);
                else
                    unfollowUser(currentUser, viewingUser);
            }
        }
    }
}
...
void viewPostSquare() // 查看帖子广场
{
    while (backToMainPage) {
        ...
        if (cmd == 0)
            ...
            viewPostDetail(POST::post[postNumber].ownerID, postNumber);
        } else if (cmd == 2) {
            launchPost(USER::user[currentUser]);
        }
    }
